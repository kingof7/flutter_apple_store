import 'package:app_store/2-inherited_widget/cart.dart';
import 'package:app_store/2-inherited_widget/state/inherited_cart.dart';
import 'package:app_store/2-inherited_widget/store.dart';
    ...
    setState(() {
      if (cartProductList.contains(product)) {
        ...
        cartProductList = cartProductList.where((element) {
          // 새로운 배열을 만들기에 참조가 달라짐
          return element != product;
        }).toList();
      } else {
        // cartProductList.add(product);
        cartProductList = [...cartProductList, product]; // 새로 생성하기에 다른 참조를 가짐
      }
    });
  }
